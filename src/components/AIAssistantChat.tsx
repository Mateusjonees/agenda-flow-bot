...
export default function AIAssistantChat({ onClose, context }: AIAssistantChatProps) {
  ...
  const chatContent = (
    <>
      ...
      >
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {isLoading && (
            ...
            >
              <div className="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full bg-purple-100 text-purple-600 dark:bg-purple-900 dark:text-purple-300">
                <BotIcon />
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-1 rounded-2xl bg-muted px-3 py-2 sm:px-4 sm:py-3">
                  <motion.span
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {quickActions.map((action, index) => (
              <button
                ...
                className="rounded-full bg-purple-100 px-2.5 py-1 text-xs text-purple-700 hover:bg-purple-200 dark:bg-purple-900 dark:text-purple-300 dark:hover:bg-purple-800 transition-colors"
              >
                {action.label}
              </button>
            ))}
      ...
      {/* Input */}
      <div className="border-t p-3 sm:p-4 pb-safe">
    ...
    return (
      <Drawer open={true} onOpenChange={(open) => !open && onClose()}>
        <DrawerContent className="h-[85vh] max-h-[85vh]">
          <DrawerHeader className="flex items-center gap-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-t-lg mx-0 px-4 py-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white/20">
              <BotIcon />
            </div>
            <div className="flex-1">
              <DrawerTitle className="text-white font-semibold text-base">Assistente IA</DrawerTitle>
