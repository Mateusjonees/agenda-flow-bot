...
export function SubscriptionStatusCard({ subscription, compact = false, pendingPix, justPaidPix }: SubscriptionStatusCardProps) {
  const navigate = useNavigate();
  
  // Verificar se é uma assinatura nova com PIX pendente
  const hasRecentSubscription = subscription && subscription.last_billing_date;
  const subscriptionDate = hasRecentSubscription ? new Date(subscription.last_billing_date!) : null;
  const isRecentSubscription = subscriptionDate ? (Date.now() - subscriptionDate.getTime()) < 300000 : false; // últimos 5 min
  const isPendingPayment = pendingPix || (subscription?.payment_method === "pix" && subscription?.status === "active" && isRecentSubscription);
  
  // Verificar se o PIX acabou de ser pago
  const isJustPaid = justPaidPix && pendingPix?.id === justPaidPix;

  if (!subscription || subscription.status === "trial") {
    // Calcular dias restantes do trial
    const trialEndDate = subscription?.next_billing_date ? new Date(subscription.next_billing_date) : null;
        ...
        <CardContent className={compact ? "pt-0" : ""}>
          <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Escolha um plano para ter acesso completo a todos os recursos após o período de teste.
            </p>
  ...
  return (
    <Card className="border-2 border-primary/20 bg-gradient-to-br from-background to-primary/5">
      ...
      <CardContent className={`space-y-3 ${compact ? "pt-0" : ""}`}>
        {/* Contagem regressiva em destaque para assinaturas ativas */}
          ...
          <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-primary/10 to-primary/5 rounded-lg border border-primary/20">
            ...
            <div className="flex-1">
              <p className="text-2xl font-bold text-primary">
                {daysRemainingActive} {daysRemainingActive === 1 ? 'dia' : 'dias'}
              </p>
              <p className="text-xs text-muted-foreground">
                restantes até renovação
              </p>
            </div>
            <div className="text-right">
              <p className="text-xs text-muted-foreground">Renova em</p>
              <p className="text-sm font-semibold">
                {format(nextBillingDate, "dd/MM/yy", { locale: ptBR })}
              </p>
            </div>
          </div>
        )}
        ...
        {subscription.payment_method && (
          <div className="flex items-center gap-2 text-sm">
            ...
            {isJustPaid ? (
              <Badge variant="outline" className="ml-2 bg-green-500/10 text-green-700 border-green-300 dark:text-green-400 dark:border-green-500 animate-pulse">
                <CheckCircle2 className="h-3 w-3 mr-1" />
                Pagamento Concluído!
              </Badge>
            ) : isPendingPayment ? (
              <Badge variant="outline" className="ml-2 bg-amber-500/10 text-amber-700 border-amber-300">
