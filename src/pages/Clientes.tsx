...
const Clientes = () => {
  ...
  const getFilteredCustomers = () => {
    ...
    return customers.filter((customer) => {
      ...
      // Buscar no telefone (só se tiver números para buscar)
      if (termNumbers && customer.phone) {
        const phoneNumbers = customer.phone.replace(/\D/g, '');
        if (phoneNumbers.includes(termNumbers)) {
          return true;
        }
      }
      ...
      // Buscar no CPF (só se tiver números para buscar)
      if (termNumbers && customer.cpf) {
        const cpfNumbers = customer.cpf.replace(/\D/g, '');
        if (cpfNumbers.includes(termNumbers)) {
          return true;
        }
      }
  ...
  const fetchCustomers = async () => {
    ...
    if (!error && data) {
      setCustomers(data);
      // Não atualiza filteredCustomers aqui - o useEffect do filtro vai cuidar disso
    } else {
